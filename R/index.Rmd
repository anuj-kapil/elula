...
## Busiest locations and postcodes
...
* Extract date and time from pickup datetimestamp
* Shift of the driver to compute daily statistics
* Compute cruise time, which is the idle time between trips
* Compute work time as total of trip time and idle time
* Calculate daily trip durations, distances, work time, daily income (total fare amount), peak operating hour

```{r eval=FALSE, include=TRUE}
trip_combined[, pickup_datetime := as.POSIXct(pickup_datetime,format="%Y-%m-%d %H:%M:%OS")]
trip_combined[, dropoff_datetime := as.POSIXct(dropoff_datetime,format="%Y-%m-%d %H:%M:%OS")]

# Extract timestamp from datetimestamp
trip_combined[,pickup_timestamp:= format(pickup_datetime, format = "%H:%M:%S")]
trip_combined[,pickup_timestamp:= as.POSIXct(pickup_timestamp, format = "%H:%M:%S")]

setorderv(trip_combined, cols = c("hack_license", "pickup_datetime"))

# Extract date part from datetimestamp
trip_combined[, work_date:= date(pickup_datetime)]

# Calculate the shift of the driver 
trip_combined[, shift_i:= findInterval(pickup_timestamp, as.POSIXct(c("00:00", "03:00", "18:00", "24:00"), format = "%H:%M"))]
trip_combined[, shift := paste(if_else(shift_i == 2L, "Day", "Night"), work_date - min(work_date) + (shift_i == 3L)), by = list(hack_license)]

# Cruise time is the idle time between trips
trip_combined[, cruise_time:= shift(pickup_datetime,1,type = "lead") - dropoff_datetime, by = list(hack_license)]
trip_combined$cruise_time <- as.numeric(trip_combined$cruise_time)

# Remove overlapping trips and extreme cruise times 90 percentile
trip_combined <- trip_combined[cruise_time>=0 & cruise_time<quantile(cruise_time, 0.9)]

trip_combined$work_time <- rowSums(trip_combined[,c('trip_time_in_secs', 'cruise_time')], na.rm = T)

trip_combined[, pickup_hour := hour(pickup_datetime)]

driver_daily <- trip_combined[, .(daily_wages = sum(total_amount), daily_work = sum(work_time), daily_trip_distance = sum(trip_distance), daily_cruise = sum(cruise_time), daily_trip_time = sum(trip_time_in_secs), median_hour = median(pickup_hour), num_trips = .N), by = list(hack_license, shift)]

driver_monthly <- driver_daily[,.(avg_income = mean(daily_wages), avg_work_time = mean(daily_work), avg_trip_distance = mean(daily_trip_distance), avg_cruise = mean(daily_cruise, na.rm = T), avg_trip_time = sum(daily_trip_time), avg_hour = mean(median_hour), avg_trips = mean(num_trips)), by = list(hack_license)]

```

## Multi-collinearity Analysis

Looking at the chart below, the average income has a fairly high linear relationship with average work time and total trips taken.

```{r eval=FALSE, include=TRUE}
driver_cormat <- driver_monthly[, list(avg_work_time,
                                       avg_trip_distance,
                                       avg_cruise,
                                       avg_trip_time,
                                       avg_hour,
...
corr <- cor(driver_cormat, use = "pairwise.complete.obs")
...
Next we try to fit a machine learning model to our driver data to characterise high income drivers from low income ones . We are going to try Extreme gradient boosting (xgboot) model for our regression problem. Here we are fitting a xgboost model with randomly chosen hyperparameters and analyse which features are important in determining the average income of driver

```{r eval=FALSE, include=TRUE}
set.seed(131)
training_samples <- createDataPartition(driver_cormat$avg_income, p = 0.8, list = FALSE)
train_data  <- driver_cormat[training_samples, ]
test_data <- driver_cormat[-training_samples, ]


# Model

xg_train <- xgb.DMatrix(data.matrix(train_data[, names(train_data)[1:6],with=FALSE])
                        , label = train_data$avg_income
                        , missing = NaN
)

xg_test <- xgb.DMatrix(data.matrix(test_data[, names(test_data)[1:6],with=FALSE])
                       , label = test_data$avg_income
                       , missing = NaN
)
...
data.table(
  RMSE = RMSE(pred, test_data$avg_income),
  R2 = R2(pred, test_data$avg_income)
)

xgb.importance(names(train_data)[1:6], model = model_4)

# Feature         Gain      Cover Frequency
# 1:     avg_work_time 0.4773516202 0.27459511 0.2310373
# 2:         avg_trips 0.2647912543 0.19359222 0.1523165
# 3: avg_trip_distance 0.2525072212 0.21180838 0.2330873
# 4:        avg_cruise 0.0029447935 0.18880676 0.1580566
# 5:     avg_trip_time 0.0018260003 0.05954298 0.1236162
# 6:          avg_hour 0.0005791105 0.07165455 0.1018860
```

The feature importance shows the top two features are *avg_work_time* and *avg_trips*

# Maximize earnings in a day
As an owner, I could think of two approaches:
* Target the peak demand hours
* Target the peak earning hours while minimising travel time
...
Further, based on the peak-times, we can sort the top pickup locations that earns the most total fare.

# Maximising 10 taxis fleet

Based on the given data:  

* Assuming a percentage share in the fare, instruct the shift drivers to target the pickups in the evenings after 4pm
...
* Not considering the competiton, the target location would be within 10kms radius from NYC center and hotspots
...
* There are errors in capturing the latitude, longitude, trip distance and trip time and we have establised that these are the key features in predicting the fare amount
* We need to establish the data quality, remediation and goverance rules around data to ensure efficient use of anytics to maximise earning
* Windsorization can be done around fare amounts, tip amounts, total amounts, trip duration and distances
* We do have oultier and data cleaning process but further rules around overlapping trips and impossible trip distance to trip time ratios can be implemented to further clean the data
* Volume of data is massive. Effecient data engineering, pre-processing and transformation can be implemented to reduce analytical infrastructure costs

# Drawbacks of current model
* Potentially overfitting to one month of trip transaction
* Seasonal effects of data/taxi trips are not getting captured as we are fitting to one month of data
* Driver characterstics are not taken in to account. A different model was attempted to characterise high income drivers
* An ensemble of model can be built to capture different dynamics of data
* Further feature engineering can be done to come up with better features
